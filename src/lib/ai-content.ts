...
export async function translateAndStructureMenu(
  ...
  const system = [
    ...
    "- LASS NIEMALS EIN GERICHT WEG. Jedes eigenständige Gericht des Tages bekommt einen eigenen Eintrag mit demselben dayOfWeek.",
    "- HAUPTGERICHT: Der Fleisch-/Fisch-Hauptteil des Tages MIT seinen Sättigungsbeilagen (Reis, Kartoffeln, Püree, Nudeln/Makkaroni) ist EIN kombinierter Eintrag mit zusammenhängendem, appetitlichem Namen (z. B. 'Гуляж з картоплею пюре. Макарони. Рис.' -> 'Gulasch mit Kartoffelpüree, Nudeln & Reis'). Sättigungsbeilagen NIE als eigene Einträge.",
    "- ZWEITGERICHT / VEGETARISCHE ALTERNATIVE: Ein eigenständiges Gericht im selben Tag (typisch vegetarisch: z. B. 'Цвітна капуста запечена в клярі' = Blumenkohl im Backteig, 'Кольрабі запечені' = ofengebackener Kohlrabi, 'Голубці по-вегетаріанськи' = vegetarische Kohlrouladen, 'Котлети з броколі' = Brokkoli-Bratlinge) ist ein SEPARATER Eintrag mit gleichem dayOfWeek – NICHT in den Namen des Hauptgerichts quetschen und NICHT weglassen. Setze dann isVegetarian (und isVegan, wenn keinerlei Tierprodukte).",
    "- Faustregel: Ein Satz/Abschnitt mit eigener Zubereitung (запечена, в клярі, котлети, голубці …) = eigenes Gericht; nackte Beilagen-Nennungen (Рис, Картопля, Макарони) = Teil des Hauptgerichts.",
    `- Wenn kein Preis genannt ist, setze price = ${defaultPrice} (einheitlicher Mittagstisch).`,
    "- Salat/Suppe, die zum Essen dazugehören, gehören in 'note' (z. B. 'mit Salat') JEDES Eintrags des Tages, nicht als eigener Eintrag.",
